...
export async function isInGitRepo(): Promise<boolean> {
	...
	try {
		const proc = await Bun.spawn(["git", "rev-parse", "--git-dir"], {
			stdout: "pipe",
			stderr: "pipe",
			cwd: process.cwd(),
		});
...
export async function getRepoRoot(): Promise<string> {
	const proc = await Bun.spawn(["git", "rev-parse", "--show-toplevel"], {
		stdout: "pipe",
		stderr: "pipe",
	});
...
export async function hasBranch(branch: string): Promise<boolean> {
	const proc = await Bun.spawn(["git", "branch", "--list", branch], {
		stdout: "pipe",
		stderr: "pipe",
	});
...
export async function getWorktreeList(): Promise<string> {
	const proc = await Bun.spawn(["git", "worktree", "list", "--porcelain"], {
		stdout: "pipe",
		stderr: "pipe",
	});
...
// EM: Implements git-worktree-isolation feature - creates worktree, branch, handles reuse and recovery
// EM: Tested by: testWorktreeBranchCreated, testExistingScoderWorktreeReused, testWorktreeRecreatedIfMissing
export async function setupGitWorktree(
	...
	if (existingWorktree && (await realpath(existingWorktree)) === repoRootReal) {
	...
	if (!foundWorktreeDir) {
		error(`Failed to locate worktree for ${worktreeBranch} after setup.`);
		process.exit(1);
	}
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (line === `branch refs/heads/${branch}`) {
			for (let j = i - 1; j >= 0; j--) {
				...
				if (prevLine?.startsWith("worktree ")) {
...
function formatRefLabel(branch: string | null, head: string): string {
